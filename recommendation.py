...
# print("images and labels: ", images, labels)
images = np.expand_dims(images, axis=3)
# Normalize the image.
images = images / 255.
# Display list of available test songs.
print( np.unique(labels) )
print("Select one of the song from the given song:")
...
for i in range(0, len(labels)):
    # print("iiiiiiiiiiiiiiiis", i)
    # print(f"Labels [] {labels[i]}")
    if(labels[i] == recommend_wrt):
        # print("labels[i] == recommend_wrt")
        test_image = images[i]
...
# print("prediction songs: ", predictions_song)
# print("prediction anchor: ", prediction_anchor)
